-- ============================================================
-- Atomic checkout v6: persist per-line discount on each order item
--
-- Adds two optional fields to the items JSON the RPC writes:
--   * discount_kzt           — how much the customer saved on this line
--   * applied_discount_name  — which rule produced the cut (snapshot of the name)
--
-- The client recomputes discounts right before submit and sends these per item.
-- The RPC still derives product details from `products` (id, name, etc.); only
-- the two discount fields are taken from the client payload.
--
-- Run AFTER atomic_checkout_v5.sql.
-- ============================================================
...
begin
  ...
  -- Aggregate per-line discount info by product_id (cart already dedupes, but be safe)
  for v_item in
    select
      parsed.product_id,
