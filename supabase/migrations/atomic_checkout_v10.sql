-- ============================================================
-- Atomic checkout v10: hybrid discounts for registered clients
--
-- Behaviour change vs v9:
--   v9 gave registered clients ONLY their personal discount and dropped
--   ALL general rules from their orders.
--
--   v10 restores general rules for clients, but keeps the масло/парфюм
--   world separate:
--     * масло / парфюм lines  -> ONLY the client's personal discount
--       (personal percent is recomputed server-side from the session;
--       per-line rule discount snapshots are cleared on those lines);
--     * other lines (original / analog / accessory) -> general rule
--       discounts as usual.
--     * both may be present on one order: discount_kzt = rules,
--       discount_sum = personal.
--
--   Rounding contract (must match src/lib/discounts.ts calcPersonalAmount):
--   for each масло/парфюм line the personal cut is computed per-line as
--       round( round(price_usd * qty * rate, 0) * percent / 100.0, 2 )
--   and then summed, so client display and the stored order agree to the
--   tenge.
--
--   Guests / staff keep the old behaviour (rules + GREATEST).
--
-- Run AFTER atomic_checkout_v9.sql.
-- ============================================================
...
begin
  ...
  loop
    ...
    -- General rule snapshots are dropped on those lines and the personal cut
    -- is accumulated per line (rounding contract — see file header).
    if v_is_client and v_product.category in ('oil', 'perfume') then
      v_line_disc := null;
      v_line_name := null;
      v_personal_sum := v_personal_sum
        + round(
            round(v_product.price_usd * v_item.quantity * p_rate_to_usd, 0)
            * v_personal_percent / 100.0,
            2
          );
    end if;

    -- Rebuild the rules total only from snapshots actually kept on lines.
    if v_line_disc is not null then
      v_rule_kzt := v_rule_kzt + v_line_disc;
    end if;
  ...
  if v_is_client then
    -- Registered clients: rules (non-oil/perfume) + personal (масло/парфюм).
    -- Both are stored on the order and may be > 0 simultaneously.
    v_discount_applied := v_rule_kzt + v_personal_sum;
    v_discount_kzt := v_rule_kzt;
    v_discount_sum := v_personal_sum;
    -- v_applied stays as sent by the client (it only contains rules).
  else
    -- Guests / staff: rule discounts + personal via GREATEST (v8/v9 behaviour).
