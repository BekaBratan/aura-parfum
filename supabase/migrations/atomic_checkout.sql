-- Atomic checkout for Aura Parfum.
-- Run this in the Supabase SQL Editor after supabase/schema.sql.
-- This does not change table schema. It creates one RPC that checks stock,
-- decrements product counts, and creates the order in a single transaction.
...
begin
  ...
  loop
    ...
    if v_product.count < v_item.quantity then
      raise exception 'Недостаточно товара в наличии: %. В корзине: %, доступно: % шт.',
        v_product.name,
        v_item.quantity,
        v_product.count;
    end if;
